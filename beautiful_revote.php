<?php
session_start();
try{
  require_once("connectCD104.php");
  $firstTitle = $_REQUEST["firstTitle"];
  $firstText = $_REQUEST["firstText"];

  $sql = "delete showitems from showitems where memNo=:memNo;";
  $vote = $pdo->prepare($sql); 
  $vote->bindValue(':memNo',$_SESSION["memNo"]);
  $vote->execute();

  $sql2 ="insert into showitems(memNo,showItemName,showItemDetail,showItemVote,showItemImg) values(:memNo,:firstTitle,:firstText,:votenumber,:memImg)";
  $vote = $pdo->prepare($sql2); 
  $vote->bindValue(':memNo',$_SESSION["memNo"]);
  $vote->bindValue(':firstTitle',$firstTitle);
  $vote->bindValue(':firstText',$firstText);
  $vote->bindValue(':votenumber',0);
  $vote->bindValue(':memImg',$_SESSION["memImg"]);
  $vote->execute();
  var_dump($firstTitle);
  var_dump($firstText);

}catch(PDOException $e){
  echo $e->getMessage();
}
?>